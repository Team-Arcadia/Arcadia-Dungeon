...
public class PlayerProgressManager {
    ...
    public List<PlayerProgress> getTopPlayers(int limit) {
        ...
        all.forEach(PlayerProgress::normalize);
        all.sort((a, b) -> {
            int levelCompare = Integer.compare(b.arcadiaProgress.arcadiaLevel, a.arcadiaProgress.arcadiaLevel);
            if (levelCompare != 0) return levelCompare;
            int xpCompare = Long.compare(b.arcadiaProgress.arcadiaXp, a.arcadiaProgress.arcadiaXp);
            if (xpCompare != 0) return xpCompare;
            return Integer.compare(b.getTotalCompletions(), a.getTotalCompletions());
        });
